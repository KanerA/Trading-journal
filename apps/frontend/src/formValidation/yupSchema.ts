...
const positionExitSchema = yup.object().shape({
    ...
    date: yup.date().required("Exit date is required").typeError("Exit date must be a valid date"),
...
export const yupSchema = yup.object().shape({
    ...
    entryDate: yup.date().required("Entry date is required").typeError("Entry date must be a valid date"),
